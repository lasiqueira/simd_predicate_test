...
inline void lp(const u8* __restrict a, size_t n, u8 b, u8* __restrict res)
{
...
// runs the scalar/AVX2/AVX512 waves for one element type and reports whether all three agreed
template <typename T>
    ...
    size_t repeatsPerSample = 1)
{
    ...
    for (size_t i = 0; i < elementCount; ++i)
    {
        if (res1[i] != res2[i] || res1[i] != res3[i])
        {
            allMatch = false;
            printf(
                "%zu: scalar = %llu, AVX2 = %llu, AVX512 = %llu\n",
                i, (unsigned long long)res1[i], (unsigned long long)res2[i], (unsigned long long)res3[i]
            );
        }
    }
...
int main()
{
    ...
    RunTypeBenchmark<u8>("u8", elementCount, (u8)100, (u8)0, (u8)255, lp, lp256, lp512, cpu_freq, seconds_per_wave);
    ...
    RunTypeBenchmark<u16>("u16", elementCount, (u16)100, (u16)1, (u16)((std::numeric_limits<u16>::max)() / 100), lp<u16>, lp256_u16, lp512_u16, cpu_freq, seconds_per_wave);
    RunTypeBenchmark<u32>("u32", elementCount, (u32)100, (u32)1, (u32)((std::numeric_limits<u32>::max)() / 100), lp<u32>, lp256_u32, lp512_u32, cpu_freq, seconds_per_wave);
    RunTypeBenchmark<u64>("u64", elementCount, (u64)100, (u64)1, (u64)((std::numeric_limits<u64>::max)() / 100), lp<u64>, lp256_u64, lp512_u64, cpu_freq, seconds_per_wave);
    ...
    RunTypeBenchmark<f32>("f32", elementCount, (f32)100, (f32)1, (f32)10000, lp<f32>, lp256, lp512, cpu_freq, seconds_per_wave);
    RunTypeBenchmark<f64>("f64", elementCount, (f64)100, (f64)1, (f64)10000, lp<f64>, lp256, lp512, cpu_freq, seconds_per_wave);
    ...
    RunTypeBenchmark<u8>("u8 (L1)", l1Bytes / sizeof(u8), (u8)100, (u8)0, (u8)255, lp, lp256, lp512, cpu_freq, seconds_per_wave, repeatsPerSample);
    RunTypeBenchmark<u16>("u16 (L1)", l1Bytes / sizeof(u16), (u16)100, (u16)1, (u16)((std::numeric_limits<u16>::max)() / 100), lp<u16>, lp256_u16, lp512_u16, cpu_freq, seconds_per_wave, repeatsPerSample);
    RunTypeBenchmark<u32>("u32 (L1)", l1Bytes / sizeof(u32), (u32)100, (u32)1, (u32)((std::numeric_limits<u32>::max)() / 100), lp<u32>, lp256_u32, lp512_u32, cpu_freq, seconds_per_wave, repeatsPerSample);
    RunTypeBenchmark<u64>("u64 (L1)", l1Bytes / sizeof(u64), (u64)100, (u64)1, (u64)((std::numeric_limits<u64>::max)() / 100), lp<u64>, lp256_u64, lp512_u64, cpu_freq, seconds_per_wave, repeatsPerSample);
    RunTypeBenchmark<f32>("f32 (L1)", l1Bytes / sizeof(f32), (f32)100, (f32)1, (f32)10000, lp<f32>, lp256, lp512, cpu_freq, seconds_per_wave, repeatsPerSample);
    RunTypeBenchmark<f64>("f64 (L1)", l1Bytes / sizeof(f64), (f64)100, (f64)1, (f64)10000, lp<f64>, lp256, lp512, cpu_freq, seconds_per_wave, repeatsPerSample);
}
